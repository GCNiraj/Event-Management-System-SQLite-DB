...
const checkAvailableSeats = async () => {
    try {
        const res = await axios({
            method: 'GET',
            url: 'http://localhost:4001/api/v1/events/availability/' + localStorage.getItem('event_id')
        });
        ...
        if (res.data.status === 'success') {
            ...
            else if (res.data.data !== null && res.data.data <= 0) {
        ...
        let message = typeof err.response !== 'undefined' ? err.response.data.message : err.message;
        showAlert('error', 'Error: Seat Unavailable', message);
        location.assign('/')
    }
}
...
const updateAvailableSeats = async (available_seats) => {
    try {
        const res = await axios({
            method: 'PATCH',
            url: 'http://localhost:4001/api/v1/events/availability/' + localStorage.getItem('event_id'),
